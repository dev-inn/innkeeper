...
import { getAllRanksForServer, getRankByRep, insertNewRank } from '../utils/rank'
...
const cmd = new Command(
  ...
  async (message, bot, args) => {
    ...
    if (ranks.length >= 25) {
      ...
      await message.reply('Sorry you can only have 25 ranks due to a discord limitation\nPerhaps remove one?')
      return
    }

    const rank = await getRankByRep(message.guild.id, entry_rep)
    ...
    try {
      await insertNewRank(message.guild.id, role, entry_rep, budget)
